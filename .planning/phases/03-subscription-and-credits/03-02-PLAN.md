...
must_haves:
  ...
  key_links:
    ...
    - from: "app/api/webhooks/stripe/route.ts"
      to: "add_overage_credits RPC"
      via: "supabase.rpc call"
      pattern: "supabase\\.rpc\\('add_overage_credits'"
---
...
5. Adds add_overage_credits() RPC function for atomic credit pack additions
  </action>
  <verify>
File exists at supabase/migrations/00002_webhook_events.sql with valid SQL syntax and contains add_overage_credits function.
  </verify>
  <done>Migration file created with webhook_events table, overage credit support, and add_overage_credits RPC function</done>
</task>
...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  // IMPORTANT: Use ONLY the add_overage_credits RPC function for atomic credit increment.
  // Do NOT use raw SQL updates or multiple query approaches.
  if (session.mode === 'payment') {
    ...
    if (pack) {
      ...
      if (sub) {
        ...
        // This is the ONLY correct approach - do not use direct table updates
...
- Idempotency via webhook_events table lookup before processing
- Credit pack handling uses ONLY `supabase.rpc('add_overage_credits', {...})` for atomic increment - no fallback approaches, no direct SQL, no alternative methods
- All event types handled in dedicated async functions
  </action>
  <verify>
1. `npx tsc --noEmit` passes
2. File exists at app/api/webhooks/stripe/route.ts
3. Exports POST function
4. Credit pack logic uses ONLY `supabase.rpc('add_overage_credits', {...})` call - no placeholder code, no commented alternatives, no fallback approaches
5. Grep for "add_overage_credits" shows exactly one call in handleCheckoutCompleted
  </verify>
  <done>Webhook handler processes checkout.session.completed, subscription updates, and invoice events with idempotency protection and clean RPC-only credit pack wiring</done>
</task>
...
- Credit pack purchase uses ONLY the add_overage_credits RPC function (no alternatives)
</verification>
...
3. Migration includes add_overage_credits RPC function
4. Updated deduct_credits and check_credits functions handle overage credits
5. Webhook handler verifies Stripe signatures using raw body
6. Webhook handler checks idempotency before processing
7. All subscription lifecycle events handled (created, updated, deleted, renewed, failed)
8. Credit pack purchases use ONLY `supabase.rpc('add_overage_credits', {...})` - no direct table updates, no alternative approaches
9. Credit transactions logged for all credit grants
</success_criteria>
